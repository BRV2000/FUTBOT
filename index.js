  ...
  sock.ev.on("connection.update", ({ connection, lastDisconnect, qr }) => {
    try {
      ...
      if (connection === "close") {
        ...
        logger.warn("🔌 Conexión cerrada. Reintentando:", shouldReconnect);
        if (shouldReconnect) connectBot();
      }
    } catch (error) {
      logger.error(`[CONNECTION_ERROR] ${error.stack}`);
    }
  });
...
¡Gracias por usar el bot! ⚽🔥`,
      ...
      logger.error(`[MSG_ERROR] ${error.stack}`);
    }
  });
}
